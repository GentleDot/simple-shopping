...
도서 상품 리소스는 상품 정보를 등록하거나 상품 설명을 수정하는 요청, 상품 정보를 조회하는 용도로 사용된다.
...
`PATCH` 요청을 사용해서 기존 도서 상품의 상품 설명을 업데이트 할 수 있다.
...
=== 카테고리별 도서 상품 조회(Get Book List by Category)
...
사용자 리소스는 사용자 등록(회원 가입), 사용자 정보 조회(회원 정보 조회) 등의 기능을 수행한다.

[[resources-members-check-email]]
=== 사용자 이메일 중복 체크(Member Email Adress Check)
...
=== 사용자 정보 조회(Member Information)

`GET` 요청을 사용해서 기존 사용자 하나의 정보를 조회한다.
...
=== 상품 주문 취소 - 주문 내역 상태 변경(Purchase Canceling)
